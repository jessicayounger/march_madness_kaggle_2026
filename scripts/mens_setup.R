...
if (!exists("common_logistic")) {
  stop("Please source('common_helpers.R') before source('mens_setup.R').")
}
...
m_file_teams <- "MTeams.csv"
m_file_slots <- "MNCAATourneySlots.csv"
m_file_seeds <- "MNCAATourneySeeds.csv"
m_file_tourn_det <- "MNCAATourneyDetailedResults.csv"
m_file_reg_det <- "MRegularSeasonDetailedResults.csv"
m_file_ratings <- "Barttorvik Away-Neutral.csv"

# ratings file column names
m_rating_season_col <- "year"
m_rating_team_col <- "team"
m_rating_value_col <- "badj_em"

m_rating_name_max_dist <- 0.15

cat("Loading men's data...\n")

# ---------------------------------------------------------
# LOAD MEN'S DATA
# ---------------------------------------------------------

m_teams <- read_csv(m_file_teams, show_col_types = FALSE) %>% clean_names()
m_slots <- read_csv(m_file_slots, show_col_types = FALSE) %>% clean_names()
m_seeds <- read_csv(m_file_seeds, show_col_types = FALSE) %>% clean_names()
m_tourn_det <- read_csv(m_file_tourn_det, show_col_types = FALSE) %>% clean_names()
m_reg_det <- read_csv(m_file_reg_det, show_col_types = FALSE) %>% clean_names()
m_ratings_raw <- read_csv(m_file_ratings, show_col_types = FALSE) %>% clean_names()

# ---------------------------------------------------------
# BUILD MEN'S RATINGS TABLE
# ---------------------------------------------------------

cat("Mapping men's ratings to team IDs...\n")

m_ratings_all <- common_map_teamnames_to_ids(
  df = m_ratings_raw,
  teams_df = m_teams,
  season_col = m_rating_season_col,
  team_col = m_rating_team_col,
  value_col = m_rating_value_col,
  max_dist = m_rating_name_max_dist
)

if (nrow(m_ratings_all) == 0) {
  stop("No men's ratings mapped. Check men's rating file and column names.")
}

# ---------------------------------------------------------
# MEN'S USABLE SEASONS
# ---------------------------------------------------------

m_years_ratings <- sort(unique(m_ratings_all$season))
m_years_slots   <- sort(unique(m_slots$season))
m_years_seeds   <- sort(unique(m_seeds$season))
m_years_tourn   <- sort(unique(m_tourn_det$season))
m_years_reg     <- sort(unique(m_reg_det$season))

m_seasons <- Reduce(intersect, list(
  m_years_ratings,
  m_years_slots,
  m_years_seeds,
  m_years_tourn,
...
if (length(m_seasons) == 0) {
  stop("No overlapping men's seasons found.")
}
...
cat("Fitting men's k_slope...\n")

m_reg_games <- m_reg_det %>%
  filter(season %in% m_seasons) %>%
  transmute(
    season,
    team_a = w_team_id,
    team_b = l_team_id,
  ...
  left_join(
    m_ratings_all %>% rename(team_a = team_id, r_a = rating),
    by = c("season", "team_a")
  ) %>%
  left_join(
    m_ratings_all %>% rename(team_b = team_id, r_b = rating),
    by = c("season", "team_b")
  ) %>%
  mutate(diff = r_a - r_b) %>%
  filter(!is.na(diff))

if (nrow(m_reg_games) == 0) {
  stop("No men's regular-season rows matched ratings.")
}

m_fit_slope <- glm(y ~ diff, data = m_reg_games, family = binomial())
m_k_slope <- unname(coef(m_fit_slope)[["diff"]])

# ---------------------------------------------------------
# MEN'S HELPERS
# ---------------------------------------------------------

m_seed_team <- function(seed_str, seeds_s) {
  seeds_s$team_id[match(seed_str, seeds_s$seed)]
}

m_make_seed_lookup <- function(season, seeds_df = m_seeds) {
  seeds_df %>%
...
m_make_rating_lookup <- function(rmap_season, season, seeds_df = m_seeds) {
  seeds_s <- seeds_df %>%
    filter(season == !!season) %>%
    transmute(team_id)
...
m_make_season_rating_lookup <- function(season, ratings_all = m_ratings_all) {
...
m_p_win <- function(teamA, teamB, rvec, k_slope = m_k_slope) {
...
m_resolve_entry_info <- function(entry, winners, seeds_s, rvec, k_slope = m_k_slope) {
  ...
  if (!is.na(ta) && !is.na(tb)) {
    p_a <- m_p_win(ta, tb, rvec, k_slope)
...
# ---------------------------------------------------------
# MEN'S BRACKET / POOL FUNCTIONS
# ---------------------------------------------------------

m_beam_map_bracket_full <- function(season, rvec, seeds_df = m_seeds, slots_df = m_slots,
                                    beam_width = 150, k_slope = m_k_slope) {
  slots_s <- slots_df %>%
    filter(season == !!season) %>%
  ...
  for (i in seq_len(nrow(slots_s))) {
    ...
    for (st in beam) {
      e1 <- m_resolve_entry_info(s1, st$winners, seeds_s, rvec, k_slope)
      e2 <- m_resolve_entry_info(s2, st$winners, seeds_s, rvec, k_slope)
      
      t1 <- e1$team_id
      t2 <- e2$team_id
      if (is.na(t1) || is.na(t2)) next
      
      p1 <- common_clamp_prob(m_p_win(t1, t2, rvec, k_slope))
    ...
    if (length(new_beam) == 0) {
      stop(paste("Men's beam collapsed at slot", slot_i, "for season", season))
    }
...
m_generate_chalk_bracket <- function(season, base_rvec, beam_width = 150, k_slope = m_k_slope) {
  m_beam_map_bracket_full(
    season = season,
    rvec = base_rvec,
    seeds_df = m_seeds,
    slots_df = m_slots,
...
m_compute_bracket_features <- function(season, picks, logp, seeds_df = m_seeds, chalk_picks) {
  seed_lu <- m_make_seed_lookup(season, seeds_df)
...
m_generate_candidate_pool <- function(season,
                                      sigma,
                                      n_candidates = 100,
                                      beam_width = 150,
                                      k_slope = m_k_slope) {
  rmap_season <- m_ratings_all %>% filter(season == !!season)
  
  if (nrow(rmap_season) < 20) {
    stop(paste("Too few men's ratings for season", season))
  }
  
  lk <- m_make_rating_lookup(rmap_season, season, m_seeds)
  base_rvec <- lk$rvec
  
  chalk <- m_generate_chalk_bracket(
    season = season,
    base_rvec = base_rvec,
    beam_width = beam_width,
    k_slope = k_slope
  ...
  for (i in seq_len(n_candidates)) {
    ...
    feats_all[[i]] <- m_compute_bracket_features(
      season = season,
      picks = br$picks,
      logp = br$logp,
      seeds_df = m_seeds,
...
m_add_pool_features <- function(picks_df, features_df) {
  champions <- picks_df %>%
    filter(rnd == 6) %>%
...
# ---------------------------------------------------------
# MEN'S REALITY / SELECTION FUNCTIONS
# ---------------------------------------------------------

m_score_bracket_against_reality <- function(season, picks_df,
                                            slots_df = m_slots,
                                            seeds_df = m_seeds,
                                            tourn_df = m_tourn_det) {
  slots_s <- slots_df %>%
    filter(season == !!season) %>%
    mutate(rnd = common_parse_slot_round(slot)) %>%
    filter(rnd >= 1, rnd <= 6) %>%
  ...
  resolve_actual_entry <- function(entry, winners, seeds_s) {
    ...
    tid <- m_seed_team(entry, seeds_s)
    if (!is.na(tid)) return(tid)
    
    ta <- m_seed_team(paste0(entry, "a"), seeds_s)
    tb <- m_seed_team(paste0(entry, "b"), seeds_s)
...
m_select_best_non1_champion <- function(feat_df) {
  non1 <- feat_df %>% filter(champ_seed > 1)
  if (nrow(non1) == 0) return(m_select_best_logp(feat_df))
  non1 %>% arrange(desc(logp), n_coinflip_picks, champ_seed, bracket_id) %>% slice(1)
}

m_select_regret_min <- function(feat_df) {
  feat_df %>%
    mutate(
      regret_score =
...
m_select_consensus_champion <- function(feat_df) {
  feat_df %>% arrange(desc(champion_support_share), desc(logp), n_coinflip_picks, bracket_id) %>% slice(1)
}

m_select_anti_consensus_champion <- function(feat_df) {
  feat_df %>%
...
m_select_stability_cluster <- function(feat_df) {
  feat_df %>% arrange(bracket_uniqueness, desc(logp), n_coinflip_picks, bracket_id) %>% slice(1)
}

m_select_balanced_contrarian <- function(feat_df) {
...
m_select_high_prob_contrarian <- function(feat_df) {
  feat_df %>%
    mutate(
...
m_evaluate_selection_rules <- function(season, feature_df, picks_df,
                                       slots_df = m_slots,
                                       seeds_df = m_seeds,
                                       tourn_df = m_tourn_det) {
  rules <- list(
    best_logp = m_select_best_logp,
    lowest_fragility = m_select_lowest_fragility,
    balanced_chalk = m_select_balanced_chalk,
    best_non1_champion = m_select_best_non1_champion,
    regret_min = m_select_regret_min,
    consensus_champion = m_select_consensus_champion,
    anti_consensus_champion = m_select_anti_consensus_champion,
    stability_cluster = m_select_stability_cluster,
    balanced_contrarian = m_select_balanced_contrarian,
    high_prob_contrarian = m_select_high_prob_contrarian
  )
  ...
  for (rule_name in names(rules)) {
    ...
    slot_correct <- m_score_bracket_against_reality(
      season = season,
...
m_classify_season_strategy <- function(champion_support_max,
                                       mean_pool_consensus,
...
cat("Men's setup ready.\n")
cat("m_k_slope =", m_k_slope, "\n")
cat("Men's seasons loaded:", paste(range(m_seasons), collapse = " to "), "\n")